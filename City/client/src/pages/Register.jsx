...
const Register = () => {
  ...
  useEffect(() => {
    // ✅ Avoid re-initializing verifier if already present
    if (!window.recaptchaVerifier) {
      window.recaptchaVerifier = new RecaptchaVerifier(auth, "recaptcha-container", {
        size: "invisible", // Change to 'normal' for debugging if needed
        callback: (response) => {
  ...
  const handleVerifyOTPAndRegister = async (e) => {
    ...
    try {
      await confirmationResult.confirm(otp.trim()); // ✅ Verifies OTP
      const formData = {
        username: name + " " + lastname,
        email,
        password,
        phoneNumber, // ✅ Match backend field name
      };
